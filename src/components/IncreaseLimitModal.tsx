...
import { increaseAdminLimit } from "@/networking/inreaseAdminLimit";
...
const IncreaseLimitModal = (
  ...
  const IncreaseLimitModal = useBoundStore((state) => state.isIncreaseLimitModalVisible);
  ...
  const handleIncreaseAdminLimit = async () => {
    const result = await increaseAdminLimit(limit);
    if (result&&result.status == 200) {
     setIncreaseLimitModal(false);
      router.back();
    }
    setIncreaseLimitModal(false);
    //   router.back();
  ...
  return (
    ...
    >
      <div className="w-[430px] h-[286px] px-[39px] pt-[72px] pb-[71px] bg-white rounded-3xl justify-center items-center inline-flex overflow-hidden">
        ...
        Set a limit for the max number of super admins that can be created
          ...
          {/*   <button
            ...
            />
            <p className="text-black/50 text-xs font-normal font-['DM Sans'] leading-[18px]">  Specify limit here </p>

</div>
            <button
              onClick={handleIncreaseAdminLimit}
